...
public class ReleaseDetailsDialog extends JDialog {
    public ReleaseDetailsDialog(Frame owner, Release release, Mp3Player mp3Player) {
        ...
        // Playback controls panel will be created and added by buildAlbumPanel
        ...
        // Delegate to album or track panel builder
        if (release instanceof Album) {
            buildAlbumPanel((Album)release, mp3Player, pnlModalContent);
        } else if (release instanceof Track) {
            buildTrackPanel((Track)release, mp3Player, pnlModalContent);
        }
    ...
    // Album-specific UI and logic
    private void buildAlbumPanel(Album album, Mp3Player mp3Player, JPanel pnlModalContent) {
        // Add playback controls panel at the bottom
        List<Track> currentTracks = DeezerClient.getAlbumTracks(album.getTracklist());
        JPanel playbackPanel = createPlaybackControlsPanel(currentTracks, mp3Player);
    ...
    // Track-specific UI and logic
    private void buildTrackPanel(Track track, Mp3Player mp3Player, JPanel pnlModalContent) {
        ...
        // Add playback controls panel for single track
        List<Track> singleTrackList = Collections.singletonList(track);
        JPanel playbackPanel = createPlaybackControlsPanel(singleTrackList, mp3Player);
        this.add(playbackPanel, BorderLayout.SOUTH);
    }

    // Playback controls panel for album
    private JPanel createPlaybackControlsPanel(List<Track> currentTracks, Mp3Player mp3Player) {
        JPanel pnlPlayer = new JPanel();
        pnlPlayer.setLayout(new BoxLayout(pnlPlayer, BoxLayout.Y_AXIS));
        pnlPlayer.setBackground(MainWindow.BG_COLOR);

        final int[] currentTrackIndex = {0};
